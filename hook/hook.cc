...
static ssize_t do_io(int fd, OriginFun fun, const char *hook_fun_name, uint32_t event, int timeout_so, Args &&...args)
{
    ...
    std::shared_ptr<moczkrin::FdCtx> ctx = moczkrin::FdMgr::GetInstance()->get(fd);
    if (!ctx)
    {
        return fun(fd, std::forward<Args>(args)...);
    }
...
retry:
    ...
    if (n == -1 && errno == EAGAIN)
    {
        ...
        // timer
        std::shared_ptr<moczkrin::Timer> timer;
        ...
        if (timeout != (uint64_t)-1)
        {
            timer = iom->addConditionTimer(timeout, [winfo, fd, iom, event]()
                ...
                iom->cancelEvent(fd, (moczkrin::IOManager::Event)(event)); }, winfo);
        }

        // 2 add event -> callback is this fiber
        int rt = iom->addEvent(fd, (moczkrin::IOManager::Event)(event));
        if (rt)
        {
...
#undef XX
    ...
    unsigned int sleep(unsigned int seconds)
    {
        ...
        std::shared_ptr<moczkrin::Fiber> fiber = moczkrin::Fiber::GetThis();
        moczkrin::IOManager *iom = moczkrin::IOManager::GetThis();
    ...
    int socket(int domain, int type, int protocol)
    {
        ...
        moczkrin::FdMgr::GetInstance()->get(fd, true);
        return fd;
    }
    ...
    int connect_with_timeout(int fd, const struct sockaddr *addr, socklen_t addrlen, uint64_t timeout_ms)
    {
        ...
        if (timeout_ms != (uint64_t)-1)
        {
            ...
            t->cancelled = ETIMEDOUT;
            iom->cancelEvent(fd, moczkrin::IOManager::WRITE); }, winfo);
        }

        int rt = iom->addEvent(fd, moczkrin::IOManager::WRITE);
        if (rt == 0)
        {
            moczkrin::Fiber::GetThis()->yield();
    ...
    {
        int fd = do_io(sockfd, accept_f, "accept", moczkrin::IOManager::READ, SO_RCVTIMEO, addr, addrlen);
        if (fd >= 0)
        {
            moczkrin::FdMgr::GetInstance()->get(fd, true);
        }
    ...
    {
        return do_io(fd, read_f, "read", moczkrin::IOManager::READ, SO_RCVTIMEO, buf, count);
    }

    ssize_t readv(int fd, const struct iovec *iov, int iovcnt)
    {
        return do_io(fd, readv_f, "readv", moczkrin::IOManager::READ, SO_RCVTIMEO, iov, iovcnt);
    }

    ssize_t recv(int sockfd, void *buf, size_t len, int flags)
    {
        return do_io(sockfd, recv_f, "recv", moczkrin::IOManager::READ, SO_RCVTIMEO, buf, len, flags);
    }

    ssize_t recvfrom(int sockfd, void *buf, size_t len, int flags, struct sockaddr *src_addr, socklen_t *addrlen)
    {
        return do_io(sockfd, recvfrom_f, "recvfrom", moczkrin::IOManager::READ, SO_RCVTIMEO, buf, len, flags, src_addr, addrlen);
    }

    ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags)
    {
        return do_io(sockfd, recvmsg_f, "recvmsg", moczkrin::IOManager::READ, SO_RCVTIMEO, msg, flags);
    }

    ssize_t write(int fd, const void *buf, size_t count)
    {
        return do_io(fd, write_f, "write", moczkrin::IOManager::WRITE, SO_SNDTIMEO, buf, count);
    }

    ssize_t writev(int fd, const struct iovec *iov, int iovcnt)
    {
        return do_io(fd, writev_f, "writev", moczkrin::IOManager::WRITE, SO_SNDTIMEO, iov, iovcnt);
    }

    ssize_t send(int sockfd, const void *buf, size_t len, int flags)
    {
        return do_io(sockfd, send_f, "send", moczkrin::IOManager::WRITE, SO_SNDTIMEO, buf, len, flags);
    }

    ssize_t sendto(int sockfd, const void *buf, size_t len, int flags, const struct sockaddr *dest_addr, socklen_t addrlen)
    {
        return do_io(sockfd, sendto_f, "sendto", moczkrin::IOManager::WRITE, SO_SNDTIMEO, buf, len, flags, dest_addr, addrlen);
    }

    ssize_t sendmsg(int sockfd, const struct msghdr *msg, int flags)
    {
        return do_io(sockfd, sendmsg_f, "sendmsg", moczkrin::IOManager::WRITE, SO_SNDTIMEO, msg, flags);
    }
    ...
    int close(int fd)
    {
        ...
        if (ctx)
        {
            auto iom = moczkrin::IOManager::GetThis();
            if (iom)
            {
                iom->cancelAll(fd);
            }
            // del fdctx
            moczkrin::FdMgr::GetInstance()->del(fd);
        }
        return close_f(fd);
    }
    ...
    int setsockopt(int sockfd, int level, int optname, const void *optval, socklen_t optlen)
    {
        ...
        if (level == SOL_SOCKET)
        {
            ...
            {
                std::shared_ptr<moczkrin::FdCtx> ctx = moczkrin::FdMgr::GetInstance()->get(sockfd);
                if (ctx)
                {
