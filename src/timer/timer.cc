...
bool Timer::refresh()
{
	...
	m_manager->m_timers.erase(it);
	m_nextTimePoint = std::chrono::system_clock::now() +
	                  std::chrono::milliseconds(m_timespan);
...
bool Timer::reset(uint64_t timespan, bool from_now)
{
	...
	auto start = from_now
	                 ...
	                 : m_nextTimePoint - std::chrono::milliseconds(m_timespan);
	m_timespan = timespan;
...
Timer::Timer(uint64_t ms, std::function<void()> cb, bool recurring,
    ...
    : m_timespan(ms), m_cb(cb), m_recurring(recurring), m_manager(manager)
{
...
{
	return addTimer(
	    ms, [this, weak_cond = std::move(weak_cond), cb = std::move(cb)]()
	    { OnTimer(weak_cond, cb); }, recurring);
...
uint64_t TimerManager::getNextTimer()
{
	...
	auto duration =
	    std::chrono::duration_cast<std::chrono::milliseconds>(timepoint - now);
...
void TimerManager::listExpiredCb(std::vector<std::function<void()>> &cbs)
{
	...
	while (!m_timers.empty() && rollover ||
	       !m_timers.empty() && (*m_timers.begin())->m_nextTimePoint <= now)
	{
		...
		{
			// 重新加入时间堆
			temp->m_nextTimePoint =
			    now + std::chrono::milliseconds(temp->m_timespan);
